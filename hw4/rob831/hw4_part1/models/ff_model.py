...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        delta_pred_normalized_numpy = ptu.to_numpy(delta_pred_normalized)
        next_obs_pred = obs_unnormalized + unnormalize(delta_pred_normalized_numpy, delta_mean, delta_std)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        loss = self.loss(preds, target)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
